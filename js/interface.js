...
try {
  (async function init() {
    const widgets = await Fliplet.Widget.findParents({ filter: { package: 'com.fliplet.dynamic-container' } });
    const dynamicContainer = widgets[0];

    if (widgets.length === 0 || !dynamicContainer.dataSourceId) {
      try {
        Fliplet.Widget.generateInterface({
          title: 'Configure data comments',
          fields: [
            {
              type: 'html',
        ...
        console.log('Failed to generate interface', error);
      }

      return;
    }

    async function manageDataSourceChange(dataSourceId) {
      if (dataSourceId) {
        toggleFields(true);

        try {
          const columns = await Fliplet.DataSources.getById(dataSourceId, {
            attributes: ['columns']
          });
          ...
          console.log('Failed to load data source columns', error);
        }
    ...
    (async function initDataSourceAndInterface() {
      try {
        ...
        const dsId = dsExist
          ? dsExist.id
          : (await Fliplet.DataSources.create({
        ...
        console.log('Failed to initialize interface', error);
      }
    })();
  })();
} catch (error) {
